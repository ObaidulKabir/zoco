...
export default function ProfilePage({ params }: { params: { orgId: string } }) {
  ...
  useEffect(() => {
    void (async () => {
      const json = await send(`/v1/orgs/${params.orgId}/profile`, undefined, loadAccess(), 'GET');
      if (json?.data) {
  ...
  return (
    <AuthShell title="Profile">
      <form
        onSubmit={async (e) => {
          e.preventDefault();
          await send(`/v1/orgs/${params.orgId}/profile`, { title, presence }, loadAccess(), 'PATCH');
        }}
      >
